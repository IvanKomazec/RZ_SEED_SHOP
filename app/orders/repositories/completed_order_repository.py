...
class CompletedOrderRepository:
    ...
    def get_all_completed_orders_by_status(self, status: str):
        completed_orders = self.db.query(CompletedOrder).filter(CompletedOrder.status == status).all()
        return completed_orders


    def get_all_completed_orders_in_specified_period_by_status(self, status: str, sy: int, sm: int, sd: int, ey: int,
                                                               em: int, ed: int):
        try:
            finalized_orders_in_specific_period = self.db.query(CompletedOrder).filter\
                (CompletedOrder.order_date.between(date(sy, sm, sd), date(ey, em, ed)), CompletedOrder.status ==
                 status).all()
    ...
    def get_completed_order_by_customer_id(self, customer_id: str):
        try:
            completed_order = self.db.query(CompletedOrder).filter(CompletedOrder.customer_id == customer_id).first()
            return completed_order
        except DatabaseError as e:
            raise e
        except InterfaceError as ee:
    ...
    def update_completed_order_status_by_id(self, completed_order_id: str, status: str = "pending",
                                            discount: bool = "false", order_date: date = date.today()):
        try:
            completed_order = self.db.query(CompletedOrder).filter(CompletedOrder.id == completed_order_id).first()
