...
class CompletedOrderService:
    ...
    def get_all_completed_orders_by_status(status: str):
        try:
            with SessionLocal() as db:
                completed_order_repository = CompletedOrderRepository(db)
                completed_orders = completed_order_repository.get_all_completed_orders_by_status(status)
                return completed_orders
        except Exception as e:
            raise e
    ...
    def get_all_completed_orders_in_specified_period_by_status(status: str, sy: int, sm: int, sd: int, ey: int,
        ...
        try:
            with SessionLocal() as db:
                ...
                completed_order_repository = CompletedOrderRepository(db)
                orders_in_specific_period = completed_order_repository.\
        ...
        try:
            with SessionLocal() as db:
                if date(sy, sm, sd) > date.today():
                    raise InvalidDateException("Provided start date is not valid", 400)
                completed_order_repository = CompletedOrderRepository(db)
                completed_orders_in_specified_period = completed_order_repository. \
                    get_all_completed_orders_in_specified_period_by_status(status, sy, sm, sd, ey, em, ed)
                order_value_list = []
    ...
    @staticmethod
    def get_product_orders_for_specified_period_by_status(status: str, sy: int, sm: int, sd: int, ey: int, em: int, ed: int):
        try:
            with SessionLocal() as db:
                if date(sy, sm, sd) > date.today():
                    raise InvalidDateException("Provided start date is not valid", 400)
                completed_order_repository = CompletedOrderRepository(db)
                completed_orders_in_specific_period = completed_order_repository. \
                    get_all_completed_orders_in_specified_period_by_status(status, sy, sm, sd, ey, em, ed)
                cart_ids = [order.cart_id for order in completed_orders_in_specific_period]
                product_order_repository = ProductOrderRepository(db)
                product_orders = []
                for cart_id in cart_ids:
                    product_orders.append(product_order_repository.get_all_product_orders_by_cart_id(cart_id))
                all_product_orders = []
                ...
                for product in all_product_orders:
                    sorted_product_orders[variety_product_repository.get_variety_name_by_id(product.variety_id)] = \
                        product.quantity
                return sorted_product_orders
        except Exception as e:
            raise e
